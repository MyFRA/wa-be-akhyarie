...
export class DeviceService {
    ...
    async update(uuid: string, updateDeviceDto: UpdateDeviceDto) {
        ...
        if (!deviceInUpdate) {
            errorHandler(400, 'Device failed to update! Record not found.');
        }
    ...
    async remove(uuid: string) {
        ...
        if (!device) {
            errorHandler(400, 'Device failed to delete! Record not found.');
        }
        ...
        try {
            if (updateStatusDeviceDto.status === 'connected') {
                ...
                if (!updateStatusDeviceDto.phone_number || !updateStatusDeviceDto.session_id || !updateStatusDeviceDto.api_key) {
                    errorHandler(400, `phone_number, session_id and api_key are required when status is connected.`);
                }
