...
export class MessageService {
    ...
    async sendText(textMessageDto: TextMessageDto) {
        ...
        if (!device) {
            errorHandler(400, 'Device not found! Failed to send message.');
        }

        // Validate contact is existing
        const contact = await this.validatorHelper.validateContact(textMessageDto.contact_uuid);
        if (!contact) {
            errorHandler(400, 'Contact not found! Failed to send message.');
        }

        // Validate contact has phone number
        if (!contact.phone_number) {
            errorHandler(400, `Contact doesn't have phone number! Failed to send message.`);
        }
    ...
    async sendAudio(audioMessageDto: AudioMessageDto) {
        ...
        if (!device) {
            errorHandler(400, 'Device not found! Failed to send audio message.');
        }

        // Validate contact is existing
        const contact = await this.validatorHelper.validateContact(audioMessageDto.contact_uuid);
        if (!contact) {
            errorHandler(400, 'Contact not found! Failed to send audio message.');
        }

        // Validate contact has phone number
        if (!contact.phone_number) {
            errorHandler(400, `Contact doesn't have phone number! Failed to send audio message.`);
        }
    ...
    async sendDocument(documentMessageDto: DocumentMessageDto) {
        ...
        if (!device) {
            errorHandler(400, 'Device not found! Failed to send document message.');
        }

        // Validate contact is existing
        const contact = await this.validatorHelper.validateContact(documentMessageDto.contact_uuid);
        if (!contact) {
            errorHandler(400, 'Contact not found! Failed to send document message.');
        }

        // Validate contact has phone number
        if (!contact.phone_number) {
            errorHandler(400, `Contact doesn't have phone number! Failed to send document message.`);
        }
    ...
    async sendImage(imageMessageDto: ImageMessageDto) {
        ...
        if (!device) {
            errorHandler(400, 'Device not found! Failed to send image message.');
        }

        // Validate contact is existing
        const contact = await this.validatorHelper.validateContact(imageMessageDto.contact_uuid);
        if (!contact) {
            errorHandler(400, 'Contact not found! Failed to send image message.');
        }

        // Validate contact has phone number
        if (!contact.phone_number) {
            errorHandler(400, `Contact doesn't have phone number! Failed to send image message.`);
        }
    ...
    async sendVideo(videoMessageDto: VideoMessageDto) {
        ...
        if (!device) {
            errorHandler(400, 'Device not found! Failed to send video message.');
        }

        // Validate contact is existing
        const contact = await this.validatorHelper.validateContact(videoMessageDto.contact_uuid);
        if (!contact) {
            errorHandler(400, 'Contact not found! Failed to send video message.');
        }

        // Validate contact has phone number
        if (!contact.phone_number) {
            errorHandler(400, `Contact doesn't have phone number! Failed to send video message.`);
        }
