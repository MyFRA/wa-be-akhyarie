...
export class MessageBroadcastService {
  ...
  async sendImage(imageMessageBroadcastDto: ImageMessageBroadcastDto) {
    ...
    if (!device) {
      errorHandler(422, 'Device not found! Failed to send broadcast image message.')
    }

    // Validate contact group is existing
    const contactGroup = await this.validatorHelper.validateContactGroup(imageMessageBroadcastDto.contact_group_uuid);
    if (!contactGroup) {
      errorHandler(422, 'Contact group not found! Failed to send broadcast image message.')
    }
    ...
    if (!contactGroupHasContacts) {
      errorHandler(422, `Contact group doesn't have contacts! Failed to send broadcast image message.`)
    }
  ...
  async sendVideo(videoMessageBroadcastDto: VideoMessageBroadcastDto) {
    ...
    if (!device) {
      errorHandler(422, 'Device not found! Failed to send broadcast video message.')
    }

    // Validate contact group is existing
    const contactGroup = await this.validatorHelper.validateContactGroup(videoMessageBroadcastDto.contact_group_uuid);
    if (!contactGroup) {
      errorHandler(422, 'Contact group not found! Failed to send broadcast video message.')
    }
    ...
    if (!contactGroupHasContacts) {
      errorHandler(422, `Contact group doesn't have contacts! Failed to send broadcast video message.`)
    }
